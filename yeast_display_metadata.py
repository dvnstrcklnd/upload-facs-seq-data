...
from util.upload_manifest import UploadManifest
...
def get_item_ids(args, nav, server):
    if args.item_ids:
        ...
        print(msg.format(output_name, operation_type, plan_id, server) + "\n")
...
def collect_data(nav, item_ids):
    ...
    st_jobs = nav.session.Job.find(st_job_ids)
...
def write_to_csv(all_data, nav):
    ...
    with open('yeast_display/manifest.csv', 'w', newline='') as csvfile:
...
def main():
    ...
    # upload_path = os.path.join(os.getcwd(), 'test_data')
    ...
    #     manifest_uri=os.path.join(upload_path, 'manifest.json'),
    #     plan_uri=os.path.join(upload_path, 'plan.json'),
    #     manifest_version=0.1,
    #     config_uri=os.path.join(upload_path, 'config.json')
    # )

    server = args.server or 'production'
    nav = Navigator(instance=server)
    
    item_ids = get_item_ids(args, nav, server)
